...
usage(void)
{
	fprint(2, "usage: rio [-b] [-B background] [-f font] [-i initcmd] [-k kbdcmd] [-s]\n");
	exits("usage");
}
	...
	for(;;)
		...
		Again:
			...
			if(w!=nil && inborder(w->screenr, mouse->xy))
				riosetcursor(corners[whichcorner(w->screenr, mouse->xy)], 1);
			else
				wsetcursor(w, FALSE);
...
sweep(void)
{
	...
	if(i == nil)
		goto Rescue;
	riosetcursor(corners[whichcorner(i->r, mouse->xy)], 1);
	goto Return;
