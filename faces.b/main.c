...
init(void)
{
	...
	bgrnd = display->black;
	arrow = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x888888FF);
	facebg = display->white;
	left = allocimage(display, leftright, GREY1, 0, DWhite);
	right = allocimage(display, leftright, GREY1, 0, DWhite);
	if(bgrnd==nil || arrow==nil || left==nil || right==nil){
	...
	if(tinyfont == nil)
		tinyfont = font;
 	mediumfont = openfont(display, "/lib/font/bit/pelm/unicode.8.font");
 	if(mediumfont == nil)
 		mediumfont = font;
	datefont = font;
...
drawtime(void)
{
	...
	string(screen, r.min, display->white, ZP, datefont, date);
}
...
drawface(Face *f, int i)
{
	...
	r.min.y = y+Facesize;
	center(mediumfont, r.min, f->str[Suser], display->white);
	r.min.y += mediumfont->height;
	tstr = facetime(f, &f->recent);
	center(mediumfont, r.min, tstr, display->white);
	if(f->unknown){
