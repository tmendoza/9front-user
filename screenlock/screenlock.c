...
redraw(void *)
{
	...
	for(;;){
		...
		dx = (Dx(screen->r) - Dx(img->r)) / 2;
		rect.min.x += dx;
		rect.max.x -= dx;
		dy = (Dy(screen->r) - Dy(img->r)) / 2;
		rect.min.y += dy;
		rect.max.y -= dy;
		draw(screen, screen->r, display->black, nil, ZP);
		draw(screen, rect, img, nil, img->r.min);
		/* identify the user on screen, centered */

		tm = localtime(time(&tp));
		s = smprint("user %s at %d:%02.2d:%02.2d", getuser(), tm->hour, tm->min, tm->sec);
		pt = subpt(pt, Pt(stringwidth(font, "m") * strlen(s) / 2, 0));
		string(screen, pt, screen->display->white, ZP, font, s);
		flushimage(display, 1);
		sleep(1000);
	}
...
lockscreen(void)
{
	...
	if((fd = open(pic, OREAD)) >= 0){
		if((img = readimage(display, fd, 0)) != nil){
 			rect = screen->r;
			pt = Pt(rect.max.x / 2, rect.max.y * 2 / 3); 
			dx = (Dx(screen->r) - Dx(img->r)) / 2;
			rect.min.x += dx;
	...
	procrfork(redraw, nil, 8*1024, RFFDG);
