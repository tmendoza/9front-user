...
drawwin(int i)
{
	...
	_string(screen, addpt(win[i].r.min, Pt(2,0)), display->white, ZP,
		font, win[i].label, nil, strlen(win[i].label), 
		win[i].r, nil, ZP, SoverD);
	border(screen, win[i].r, 1, display->white, ZP);	
	win[i].dirty = 0;
}
...
main(int argc, char **argv)
{
	...
	if(initdraw(0, fontname, "winwatch") < 0)
		sysfatal("initdraw: %r");
	lightblue = allocimagemix(display, DBlack, DBlack);

	statecol[0] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xCCCCCCFF);
	statecol[1] = lightblue;
	statecol[2] = lightblue;
	statecol[3] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, DMagenta);
